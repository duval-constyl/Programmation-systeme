...
namespace RestaurantGroupe9
{
    class ControllerCommande
    {
        ...
        public ControllerCommande(Model model)
        {
            this.model = model;
        }
        ...
        {
            model.gestionCuisine.commande.Add(new Commande());
            Random rnd = new Random();
            int nbOrder = rnd.Next(0, 8);
            cli.commander = true;
            for (int i = 0; i < nbOrder; i++)
            {
                model.gestionCuisine.commande[0].plats.Add(model.menu.plats[i]);
                Console.WriteLine(model.gestionCuisine.commande[0].plats[i] + " à été ajouté ");
            }
        }
        ...
        public void GoToKitchenVAISSELLE(Sprite sprite)
        {
            ...
            sprite.MovementSpeed = Restaurant.speed;
        }
        ...
        public void GoToTable(Sprite sprite, int nbTable)
        {
            foreach (Carre carre in model.gestionReception.carre)
            {
                foreach (Rang rang in carre.rang)
                {
                    ...
                    {
                        if (table.nombreTable == nbTable)
                        {
                            Console.WriteLine(sprite.SpriteName + " va à la table " + table.nombreTable);
        ...
        public void GoToButler(Sprite sprite)
        {
            ...
            sprite.MoveTo(new Point(model.gestionReception.maitreHotel.posX + 32, model.gestionReception.maitreHotel.posY));
            sprite.AutomaticallyMoves = true;
            sprite.MovementSpeed = Restaurant.speed;
            while (!sprite.SpriteReachedEndPoint) { }
        }
